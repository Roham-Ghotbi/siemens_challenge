...
if __name__ == "__main__":
	...
	def get_action():
			...
			if "action" in traj:
				...
				info = traj["info"]
				succ = traj["success"]
				return (action, info, succ)

	def update_times(traj):
		global execute_grasp_times
		c_img = traj["c_img"]
		d_img = traj["d_img"]
		c_after = None
		if "c_img_result" in traj:
	...
	def update_stops(traj):
		#some rollouts were made before this statistic was added
		global total_stopped
		global completions
		global false_completions
		global grasp_crashes
		global singulate_crashes
		...
		def update_grasp_stats(action, info, succ, traj):
			global grasp_attempts
			global color_attempts
			global rollout_grasps
			global grasp_successes
			global curr_grasp_sequence_t
			global curr_grasp_sequence_s
			global prev_singulate_info
			global color_successes
			global fail_num
...
def update_singulation_stats():
	global curr_grasp_sequence_s
	global curr_grasp_sequence_t
	global curr_singulate_sequence
	global singulation_attempts
	global prev_singulate_info
